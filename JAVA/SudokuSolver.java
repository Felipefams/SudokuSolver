				...
				if(board[row][column] == 0)
				{
					...
					for(int numberToTry = 1; numberToTry <= GRID_SIZE; ++numberToTry)
					{
						if(isValidPlacement(board,numberToTry,row,column))
						{
							...
							//call the function recusively to travel the entire board again checking for empty spots
							if(solveBoard(board))
							{
								return true;
							}
							...
							{
								//if there's no way to solve the rest of the board with numberToTry in position [row][column]
